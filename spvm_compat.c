...
void SPVM_COMPAT_sv_catpvn(SPVM_SV* sv, const char* pv, size_t length) {
  char* new_buffer = SPVM_UTIL_ALLOCATOR_safe_malloc_i32(sv->length + length + 1, sizeof(char));
...
SPVM_SV* SPVM_COMPAT_newSVpvn(SPVM* spvm, const char* pv, size_t length) {
  ...
  sv->buffer = SPVM_UTIL_ALLOCATOR_safe_malloc_i32(length + 1, sizeof(int8_t));
