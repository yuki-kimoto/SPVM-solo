  ...
  while(1) {
    ...
    switch(c) {
      ...
      case '"': {
        ...
        constant->resolved_type = SPerl_HASH_search(sperl, parser->resolved_type_symtable, "string", strlen("string"));
        op->uv.constant = constant;
      ...
      default:
        ...
        // Keyname or name
          ...
          if (memcmp(keyword, "my", str_len) == 0) {
            yylvalp->opval = SPerl_TOKE_newOP(sperl, SPerl_OP_C_CODE_MY_VAR);
            return MY;
          }
          else if (memcmp(keyword, "has", str_len) == 0) {
            yylvalp->opval = SPerl_TOKE_newOP(sperl, SPerl_OP_C_CODE_FIELD);
            return HAS;
          }
