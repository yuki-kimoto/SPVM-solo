      ...
      case '\0':
        ...
        while (1) {
          ...
          if (use) {
            ...
            SPerl_CLASS* found_class = SPerl_HASH_search(parser->type_symtable, type_name, strlen(type_name));
            if (found_class) {
              continue;
            }
            else {
              ...
              for (SPerl_int i = 0; i < parser->include_pathes->length; i++) {
                ...
                // Change :: to /
                SPerl_char* type_name_for_path = SPerl_PARSER_new_string(parser, strlen(type_name));
                SPerl_char* bufptr_orig = type_name;
                SPerl_char* bufptr_to = type_name_for_path;
                while (*bufptr_orig) {
                ...
                *bufptr_orig = '\0';
                ...
                // File name
                cur_file = SPerl_PARSER_new_string(parser, strlen(include_path) + strlen(type_name_for_path) + 6);
                sprintf(cur_file, "%s/%s.spvm", include_path, type_name_for_path);
              ...
              if (!fh) {
                if (use->op) {
                  fprintf(stderr, "Can't find package \"%s\" at %s line %d\n", use->type_name_word->value, use->op->file, use->op->line);
                }
                else {
              ...
              parser->current_use_type_name = type_name;
              parser->cur_line = 1;
              break;
            }
          }
        ...
        // Keyword or word
          ...
          if (expect != SPerl_TOKE_C_EXPECT_WORD) {
            if (memcmp(keyword, "my", str_len) == 0) {
              ...
              op->uv.pv = SPerl_ARRAY_fetch(parser->types, SPerl_TYPE_CORE_C_CODE_BOOLEAN);
              yylvalp->opval = op;
              ...
              SPerl_OP* op = _newOP(parser, SPerl_OP_C_CODE_CORETYPE);
              op->uv.pv = SPerl_ARRAY_fetch(parser->types, SPerl_TYPE_CORE_C_CODE_CHAR);
              yylvalp->opval = op;
              ...
              SPerl_OP* op = _newOP(parser, SPerl_OP_C_CODE_CORETYPE);
              op->uv.pv = SPerl_ARRAY_fetch(parser->types, SPerl_TYPE_CORE_C_CODE_BYTE);
              yylvalp->opval = op;
              ...
              SPerl_OP* op = _newOP(parser, SPerl_OP_C_CODE_CORETYPE);
              op->uv.pv = SPerl_ARRAY_fetch(parser->types, SPerl_TYPE_CORE_C_CODE_SHORT);
              yylvalp->opval = op;
              ...
              SPerl_OP* op = _newOP(parser, SPerl_OP_C_CODE_CORETYPE);
              op->uv.pv = SPerl_ARRAY_fetch(parser->types, SPerl_TYPE_CORE_C_CODE_INT);
              yylvalp->opval = op;
              ...
              SPerl_OP* op = _newOP(parser, SPerl_OP_C_CODE_CORETYPE);
              op->uv.pv = SPerl_ARRAY_fetch(parser->types, SPerl_TYPE_CORE_C_CODE_LONG);
              yylvalp->opval = op;
              ...
              SPerl_OP* op = _newOP(parser, SPerl_OP_C_CODE_CORETYPE);
              op->uv.pv = SPerl_ARRAY_fetch(parser->types, SPerl_TYPE_CORE_C_CODE_FLOAT);
              yylvalp->opval = op;
              ...
              SPerl_OP* op = _newOP(parser, SPerl_OP_C_CODE_CORETYPE);
              op->uv.pv = SPerl_ARRAY_fetch(parser->types, SPerl_TYPE_CORE_C_CODE_DOUBLE);
              yylvalp->opval = op;
          ...
          if (expect == SPerl_TOKE_C_EXPECT_PACKAGENAME) {
            ...
            // Class
              ...
              if (strcmp(keyword, parser->current_use_type_name) != 0) {
                fprintf(stderr, "Package name \"%s\" must be \"%s\" at %s line %d\n",
                  keyword, parser->current_use_type_name, parser->cur_file, parser->cur_line);
                exit(1);
              }
            }
          }
