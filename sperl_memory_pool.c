...
SPerl_MEMORY_POOL* SPerl_MEMORY_POOL_new(SPerl* sperl) {
  ...
  SPerl_MEMORY_POOL* memory_pool = (SPerl_MEMORY_POOL*) SPerl_ALLOCATOR_PARSER_safe_malloc_zero(sperl, sperl->parser, 1, sizeof(SPerl_MEMORY_POOL));
  memory_pool->base_capacity = 0xFF;
  
  SPerl_MEMORY_POOL_PAGE* page = (SPerl_MEMORY_POOL_PAGE*)SPerl_MEMORY_POOL_PAGE_new(sperl);
  page->data = (uint8_t*) SPerl_ALLOCATOR_PARSER_safe_malloc_zero(sperl, sperl->parser, memory_pool->base_capacity, sizeof(uint8_t));
...
void* SPerl_MEMORY_POOL_alloc(SPerl* sperl, SPerl_MEMORY_POOL* memory_pool, int32_t byte_size) {
  ...
  if (current_pos + aligned_byte_size > current_capacity) {
    ...
    new_page->data = (uint8_t*) SPerl_ALLOCATOR_PARSER_safe_malloc_zero(sperl, sperl->parser, new_capacity, sizeof(uint8_t));
