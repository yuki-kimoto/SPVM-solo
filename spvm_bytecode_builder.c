...
void SPVM_BYTECODE_BUILDER_push_inc_bytecode(SPVM_COMPILER* compiler, SPVM_BYTECODE_ARRAY* bytecode_array, SPVM_OP* op_inc, int32_t value) {
  ...
  SPVM_RESOLVED_TYPE* resolved_type = SPVM_OP_get_resolved_type(compiler, op_inc);
...
void SPVM_BYTECODE_BUILDER_build_bytecode_array(SPVM_COMPILER* compiler) {
  ...
  for (int32_t package_pos = 0; package_pos < compiler->op_packages->length; package_pos++) {
    SPVM_OP* op_package = SPVM_ARRAY_fetch(compiler->op_packages, package_pos);
          ...
          while (1) {
            ...
            switch (op_cur->code) {
              case SPVM_OP_C_CODE_STORE: {
                ...
                int32_t my_var_index = op_my_var->uv.my_var->index;
                
                // Currently only use catch block argument
                if (!SPVM_RESOLVED_TYPE_is_numeric(compiler, resolved_type)) {
                  SPVM_BYTECODE_ARRAY_push(compiler, bytecode_array, SPVM_BYTECODE_C_CODE_STORE_ADDRESS);
                  if (my_var_index > 0xFF) {
                    SPVM_BYTECODE_ARRAY_push(compiler, bytecode_array, (my_var_index >> 8) & 0xFF);
                    SPVM_BYTECODE_ARRAY_push(compiler, bytecode_array, my_var_index);
                  }
              ...
              case SPVM_OP_C_CODE_SWITCH: {
                ...
                SPVM_RESOLVED_TYPE* term_condition_resolved_type = SPVM_OP_get_resolved_type(compiler, op_term_condition);
                ...
                // lookupswitch
                  ...
                  if (term_condition_resolved_type->id == SPVM_RESOLVED_TYPE_C_ID_INT) {
                    ...
                    *(int32_t*)&bytecode_array->values[cur_switch_bytecode_index + padding + 1] = default_offset;
                    ...
                    SPVM_ARRAY* ordered_op_cases = SPVM_COMPILER_ALLOCATOR_alloc_array(compiler, compiler->allocator, 0);
                    for (int32_t i = 0; i < length; i++) {
              ...
              case SPVM_OP_C_CODE_CALL_FIELD: {
                ...
                if (!op_cur->lvalue) {
                  SPVM_RESOLVED_TYPE* resolved_type = SPVM_OP_get_resolved_type(compiler, op_cur);
                  ...
                  if (resolved_type->id == SPVM_RESOLVED_TYPE_C_ID_BYTE) {
                    ...
                    SPVM_BYTECODE_ARRAY_push(compiler, bytecode_array, SPVM_BYTECODE_C_CODE_GET_FIELD_ADDRESS);
                  }
              ...
              case SPVM_OP_C_CODE_CONDITION: {
                ...
                if (op_condition_target->code == SPVM_OP_C_CODE_UNDEF) {
                  ...
                  SPVM_RESOLVED_TYPE* resolved_type = SPVM_OP_get_resolved_type(compiler, op_condition_target->first);
                  if (!resolved_type) {
                    resolved_type = SPVM_OP_get_resolved_type(compiler, op_condition_target->last);
                  }
                  
                  if (!resolved_type) {
                    SPVM_BYTECODE_ARRAY_push(compiler, bytecode_array, SPVM_BYTECODE_C_CODE_IF_EQ_CMP_ADDRESS);
                  }
                  ...
                  if (!resolved_type) {
                    SPVM_BYTECODE_ARRAY_push(compiler, bytecode_array, SPVM_BYTECODE_C_CODE_IF_NE_CMP_ADDRESS);
                  }
                  else if (resolved_type->id == SPVM_RESOLVED_TYPE_C_ID_BYTE) {
                    SPVM_BYTECODE_ARRAY_push(compiler, bytecode_array, SPVM_BYTECODE_C_CODE_CMP_BYTE);
                  ...
                  if (op_condition_target->code == SPVM_OP_C_CODE_IF) {
                    ...
                    SPVM_RESOLVED_TYPE* resolved_type = SPVM_OP_get_resolved_type(compiler, op_condition_target);
                    if (resolved_type->id == SPVM_RESOLVED_TYPE_C_ID_BYTE) {
              ...
              case SPVM_OP_C_CODE_MALLOC: {
                SPVM_RESOLVED_TYPE* resolved_type = SPVM_OP_get_resolved_type(compiler, op_cur->first);
                
                if (SPVM_RESOLVED_TYPE_is_array(compiler, resolved_type)) {
                  SPVM_BYTECODE_ARRAY_push(compiler, bytecode_array, SPVM_BYTECODE_C_CODE_MALLOC_ARRAY);
                  if (SPVM_RESOLVED_TYPE_is_array_numeric(compiler, resolved_type)) {
                    if (strcmp(resolved_type->name, "byte[]") == 0) {
                      SPVM_BYTECODE_ARRAY_push(compiler, bytecode_array, SPVM_DATA_ARRAY_C_VALUE_TYPE_BYTE);
                    }
                    else if (strcmp(resolved_type->name, "short[]") == 0) {
                      SPVM_BYTECODE_ARRAY_push(compiler, bytecode_array, SPVM_DATA_ARRAY_C_VALUE_TYPE_SHORT);
                    }
                    else if (strcmp(resolved_type->name, "int[]") == 0) {
                      SPVM_BYTECODE_ARRAY_push(compiler, bytecode_array, SPVM_DATA_ARRAY_C_VALUE_TYPE_INT);
                    }
                    else if (strcmp(resolved_type->name, "long[]") == 0) {
                      SPVM_BYTECODE_ARRAY_push(compiler, bytecode_array, SPVM_DATA_ARRAY_C_VALUE_TYPE_LONG);
                    }
                    else if (strcmp(resolved_type->name, "float[]") == 0) {
                      SPVM_BYTECODE_ARRAY_push(compiler, bytecode_array, SPVM_DATA_ARRAY_C_VALUE_TYPE_FLOAT);
                    }
                    else if (strcmp(resolved_type->name, "double[]") == 0) {
                      SPVM_BYTECODE_ARRAY_push(compiler, bytecode_array, SPVM_DATA_ARRAY_C_VALUE_TYPE_DOUBLE);
                    }
                    else {
                      assert(0);
                    }
                    ...
                    SPVM_BYTECODE_ARRAY_push(compiler, bytecode_array, SPVM_DATA_ARRAY_C_VALUE_TYPE_REF);
                  }
                }
                else {
              ...
              case SPVM_OP_C_CODE_ARRAY_ELEM: {
                ...
                if (resolved_type->id == SPVM_RESOLVED_TYPE_C_ID_BYTE)
                {
                  ...
                  SPVM_BYTECODE_ARRAY_push(compiler, bytecode_array, SPVM_BYTECODE_C_CODE_ARRAY_LOAD_ADDRESS);
                }
              ...
              case SPVM_OP_C_CODE_ASSIGN: {
                ...
                if (op_cur->first->code == SPVM_OP_C_CODE_VAR) {
                  ...
                  SPVM_RESOLVED_TYPE* resolved_type = SPVM_OP_get_resolved_type(compiler, op_var);
                  ...
                  SPVM_RESOLVED_TYPE* resolved_type = SPVM_OP_get_resolved_type(compiler, op_array_elem);
                  SPVM_RESOLVED_TYPE* index_resolved_type = SPVM_OP_get_resolved_type(compiler, op_term_index);
                  ...
                  if (resolved_type->id == SPVM_RESOLVED_TYPE_C_ID_BYTE) {
                    ...
                    SPVM_BYTECODE_ARRAY_push(compiler, bytecode_array, SPVM_BYTECODE_C_CODE_ARRAY_STORE_ADDRESS);
                  }
                }
                  ...
                  if (resolved_type->id == SPVM_RESOLVED_TYPE_C_ID_BYTE) {
                    ...
                    SPVM_BYTECODE_ARRAY_push(compiler, bytecode_array, SPVM_BYTECODE_C_CODE_SET_FIELD_ADDRESS);
                  }
              ...
              case SPVM_OP_C_CODE_CONVERT: {
                ...
                SPVM_RESOLVED_TYPE* src_resolved_type = SPVM_OP_get_resolved_type(compiler, op_src_term);
                SPVM_RESOLVED_TYPE* dist_resolved_type = SPVM_OP_get_resolved_type(compiler, op_dist_type);
              ...
              case SPVM_OP_C_CODE_POP: {
                ...
                if (op_first->code != SPVM_OP_C_CODE_ASSIGN && op_first->code != SPVM_OP_C_CODE_RETURN && !op_first->lvalue) {
                  SPVM_RESOLVED_TYPE* resolved_type = SPVM_OP_get_resolved_type(compiler, op_first);
                  
                  if (SPVM_RESOLVED_TYPE_is_numeric(compiler, resolved_type)) {
                    SPVM_BYTECODE_ARRAY_push(compiler, bytecode_array, SPVM_BYTECODE_C_CODE_POP);
                  }
                  else {
                    SPVM_BYTECODE_ARRAY_push(compiler, bytecode_array, SPVM_BYTECODE_C_CODE_POP_ADDRESS);
                  }
                }
