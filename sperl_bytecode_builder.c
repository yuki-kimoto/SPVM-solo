...
void SPerl_BYTECODE_BUILDER_push_load_bytecode(SPerl* sperl, SPerl_BYTECODE_ARRAY* bytecode_array, SPerl_OP* op_var) {
  ...
  if (has_operand) {
    if (my_var_pos > 0xFF) {
      SPerl_BYTECODE_ARRAY_push(sperl, bytecode_array, (my_var_pos >> 8) & 0xFF);
      SPerl_BYTECODE_ARRAY_push(sperl, bytecode_array, my_var_pos);
    }
    else {
      SPerl_BYTECODE_ARRAY_push(sperl, bytecode_array, my_var_pos);
    }
            ...
            switch (op_cur->code) {
              ...
              case SPerl_OP_C_CODE_CONSTANT: {
                ...
                if (constant->code == SPerl_CONSTANT_C_CODE_BYTE) {
                  ...
                  SPerl_BYTECODE_ARRAY_push(sperl, bytecode_array, (constant->address >> 24) & 0xFF);
                  SPerl_BYTECODE_ARRAY_push(sperl, bytecode_array, (constant->address >> 16) & 0xFF);
                  SPerl_BYTECODE_ARRAY_push(sperl, bytecode_array, (constant->address >> 8) & 0xFF);
                  SPerl_BYTECODE_ARRAY_push(sperl, bytecode_array, constant->address & 0xFF);
                  bytecode_set = 1;
                }
