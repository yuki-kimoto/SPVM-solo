...
SPVM_RUNTIME* SPVM_RUNTIME_new(SPVM* spvm) {
  ...
  assert(sizeof(SPVM_DATA_STRING) <= SPVM_DATA_C_HEADER_BYTE_SIZE);
...
void SPVM_RUNTIME_call_sub(SPVM* spvm, SPVM_RUNTIME* runtime, int32_t sub_constant_pool_address) {
  ...
  case_SPVM_BYTECODE_C_CODE_MALLOC_OBJECT: {
    ...
    // int32_t field_index = SPVM_DATA_API_get_object_field_index(data_object, "x");
    // warn("AAAAAAAAAAAA %d", field_index);
