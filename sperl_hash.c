...
void SPerl_HASH_maybe_extend_key_buffer(SPerl* sperl, SPerl_HASH* hash, int32_t length) {
  (void)sperl;
      ...
      if (strncmp(key, &hash->key_buffer[next_entry->key_index], length) == 0) {
        *(void**)&next_entry->value = value;
        ...
        if (next_entry->next_index != -1) {
          ...
          int32_t new_entry_index = SPerl_HASH_new_hash_entry(sperl, hash, key, value);
