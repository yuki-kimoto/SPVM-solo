...
SPerl_BYTECODE_ARRAY* SPerl_BYTECODE_ARRAY_new(SPerl* sperl) {
  ...
  SPerl_BYTECODE_ARRAY* bytecodes = SPerl_ALLOCATOR_UTIL_safe_malloc_i32(1, sizeof(SPerl_BYTECODE_ARRAY));
  bytecodes->capacity = 64;
  bytecodes->length = 0;
  
  uint8_t* values = SPerl_ALLOCATOR_UTIL_safe_malloc_i32(bytecodes->capacity, sizeof(uint8_t));
...
void SPerl_BYTECODE_ARRAY_push(SPerl* sperl, SPerl_BYTECODE_ARRAY* bytecodes, uint8_t value) {
  ...
  if (length >= capacity) {
    ...
    bytecodes->values = (uint8_t*) SPerl_ALLOCATOR_UTIL_safe_realloc_i32(bytecodes->values, new_capacity, sizeof(uint8_t));
