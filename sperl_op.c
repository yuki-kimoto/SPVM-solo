...
void SPerl_OP_insert_op_convert(SPerl_PARSER* parser, SPerl_OP* op) {
  ...
  if (first_type_id != last_type_id) {
    ...
    SPerl_OP* op_convert = SPerl_OP_newOP(parser, SPerl_OP_C_CODE_CONVERT, op->file, op->line);
    
    if (first_type_id == SPerl_RESOLVED_TYPE_C_ID_INT) {
      if (last_type_id == SPerl_RESOLVED_TYPE_C_ID_LONG) {
        op_convert->uv.resolved_type = SPerl_HASH_search(parser->resolved_type_symtable, "long", strlen("long"));
        replace_first = 1;
      }
      else if (last_type_id == SPerl_RESOLVED_TYPE_C_ID_FLOAT) {
        op_convert->uv.resolved_type = SPerl_HASH_search(parser->resolved_type_symtable, "float", strlen("float"));
        replace_first = 1;
      }
      else if (last_type_id == SPerl_RESOLVED_TYPE_C_ID_DOUBLE) {
        op_convert->uv.resolved_type = SPerl_HASH_search(parser->resolved_type_symtable, "double", strlen("double"));
        replace_first = 1;
      }
    }
...
SPerl_OP* SPerl_OP_build_field(SPerl_PARSER* parser, SPerl_OP* op_var, SPerl_OP* op_field_name) {
  ...
  SPerl_OP_sibling_splice(parser, op_field, op_var, 0, op_field_name);
...
SPerl_OP* SPerl_OP_build_decl_my(SPerl_PARSER* parser, SPerl_OP* op_my, SPerl_OP* op_var, SPerl_OP* op_type) {
  ...
  // Name OP
  SPerl_OP* op_name = SPerl_OP_newOP(parser, SPerl_OP_C_CODE_NAME, op_var->file, op_var->line);
...
SPerl_OP* SPerl_OP_build_decl_enum(SPerl_PARSER* parser, SPerl_OP* op_enum, SPerl_OP* op_enum_block) {
  ...
  while ((op_enumeration_value = SPerl_OP_sibling(parser, op_enumeration_value))) {
    ...
    // sub name
    SPerl_OP* op_sub_name = SPerl_OP_newOP(parser, SPerl_OP_C_CODE_NAME, op_enumeration_value->file, op_enumeration_value->line);
...
SPerl_OP* SPerl_OP_build_type_name(SPerl_PARSER* parser, SPerl_OP* op_name) {
  ...
  SPerl_OP* op_type_name = SPerl_OP_newOP(parser, SPerl_OP_C_CODE_TYPE, op_name->file, op_name->line);
  SPerl_OP_sibling_splice(parser, op_type_name, NULL, 0, op_name);
